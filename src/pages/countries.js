...
const CountryPage = ({ data }) => {
  ...
  return (
    <Layout>
      <SEO
        title="Worldwide Online Radio Streams Sorted On Country"
        lang={ "en" }
      ...
      <Container maxWidth={ "xl" } className={ classes.root }>
        ...
        <Grid container={ true } spacing={ 4 } direction={ "row" }
          ...
          { data.allContentfulRadioStation.group.map((node, index) => {
            const country = node.edges[ 0 ].node.stationCountry.countryTitle
            return (
              <Grid item xs={ 12 } sm={ 6 } md={ 4 } lg={ 3 } key={ index }>
                <CategorySection data={ node } title={ country }/>
              </Grid>
            )
          }) }
        </Grid>
...
export const CountryQuery = graphql`
    query {
        allContentfulRadioStation(sort: {fields: stationTitle, order: ASC}, filter: {node_locale: {eq: "en"}}) {
            group(field: stationCountry___id) {
                edges {
                    node {
                        stationTitle
                        stationSlug
